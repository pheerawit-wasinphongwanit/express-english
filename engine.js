...
// ALL game logic lives in core.js; ALL content lives in bank-*.js + data.js (docs/story.md is the source of truth).
(function () {
'use strict';
const D = window.EXPRESS_DATA, C = window.EXPRESS_CORE, K = D.keys;
const BANK = [].concat(window.EXPRESS_BANK_GRAMMAR, window.EXPRESS_BANK_VOCAB, window.EXPRESS_BANK_CONTEXT);
...
let q = null;            // current question (served by core.makeQuestion)
let usedIds = new Set(); // no-repeat-in-run (docs/story.md sampler rule)
...
function startRun(m) {
  ...
  usedIds = new Set();
  rng = m === 'daily'
    ? C.mulberry32(C.hashSeed('express-english:' + dateKey()))
...
function nextQuestion() {
  q = C.makeQuestion(rng, D.waves[S.wave], BANK, usedIds);
  usedIds.add(q.id);

  // context: passage card slides up before the Thai stem (docs/story.md feedback loop)
  $('passage').hidden = !q.passage;
  $('passage').textContent = q.passage || '';
  $('question').textContent = q.passage ? q.stem : q.prompt;
  $('formHint').textContent = q.form === 'spot' ? CP.play.spotHint
    : q.form === 'fill' ? CP.play.fillHint : '';
...
function answer(i) {
  ...
  const reward = q.form === 'context' ? D.contextRewardSec : undefined; // engine default = wave reward
  const events = C.applyAnswer(S, D, correct, reward);
  const btns = [...$('choices').children];
  btns[i].classList.add(correct ? 'good' : 'bad');
  if (!correct) btns[q.correctIndex].classList.add('reveal'); // ✓ on the right answer (~1s learning moment)
  ...
  if (S.over) { setTimeout(endRun, 900); return; }
  ...
  // wrong answers hold ~1s so the revealed ✓ sinks in; correct flow on quickly
  setTimeout(() => { if (!S.over) nextQuestion(); }, correct ? 350 : 1000);
}
